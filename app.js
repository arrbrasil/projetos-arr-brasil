...
let page=1,filtered=[],drill=null;const fields=['registry','status','category','method'];
...
function groupName(status){const s=(status||'').toLowerCase();if(s==='registered'||s.includes('certified project')||s.includes('certificates issued'))return'Registrado, certificado ou emitido';if(s.includes('withdraw')||s.includes('reject')||s.includes('inactive')||s.includes('on hold')||s.includes('denied'))return'Interrompido ou não aprovado';if(s.includes('validation')||s.includes('approval requested')||s.includes('registration requested')||s.includes('certified design'))return'Em validação ou aprovação';if(s.includes('pipeline')||s==='listed')return'Em desenvolvimento';return'Outras situações'}
function grouped(){const o={};filtered.forEach(x=>{const g=groupName(x.status);o[g]=(o[g]||0)+1});return Object.entries(o).sort((a,b)=>b[1]-a[1])}
...
function applyDrill(label,test){drill={label,test};page=1;render();switchTab('projetos');window.scrollTo({top:document.querySelector('.tabs').offsetTop-10,behavior:'smooth'})}
function bars(id,data,color='#1f7a59',makeTest=null){const max=Math.max(...data.map(x=>x[1]),1);$(id).innerHTML=data.map((x,n)=>'<button class="bar chartButton" data-n="'+n+'" title="Ver projetos deste grupo"><div class="barMeta"><span title="'+esc(x[0])+'">'+esc(x[0])+'</span><b>'+fmt(x[1])+'</b></div><div class="track"><i style="width:'+(x[1]/max*100)+'%;background:'+color+'"></i></div></button>').join('')||'<p class="empty">Nenhum dado no recorte.</p>';if(makeTest)$(id).querySelectorAll('.chartButton').forEach(b=>b.onclick=()=>{const label=data[+b.dataset.n][0];applyDrill(label,makeTest(label))})}
...
function render(){
 ...
 filtered=PROJECTS.filter(x=>fields.every(k=>$(k).value==='Todos'||x[k]===$(k).value)&&(!q||[x.id,x.name,x.developer,x.state].some(v=>(v||'').toLowerCase().includes(q)))&&(!drill||drill.test(x)));
 ...
 bars('registries',count('registry'),'#1f7a59',v=>x=>x.registry===v);bars('groupedStatuses',grouped(),'#d29a45',v=>x=>groupName(x.status)===v);bars('statuses',count('status',8),'#d29a45',v=>x=>(x.status||'Não informado')===v);
 bars('categories',count('category',10),'#397f68',v=>x=>(x.category||'Não informado')===v);bars('methods',count('method',10),'#8d6d38',v=>x=>(x.method||'Não informado')===v);bars('developers',count('developer',10),'#55766b',v=>x=>(x.developer||'Não informado')===v);
 bars('states',count('state',12),'#1f7a59',v=>x=>(x.state||'Não informado')===v);
 ...
 $('results').textContent=fmt(filtered.length)+' resultados';$('drillChip').hidden=!drill;$('drillChip').innerHTML=drill?'Recorte do gráfico: <strong>'+esc(drill.label)+'</strong><button id="removeDrill">Remover ×</button>':'';if(drill)$('removeDrill').onclick=()=>{drill=null;page=1;render()};
 ...
 document.querySelectorAll('tbody tr').forEach(tr=>tr.onclick=()=>detail(filtered[+tr.dataset.i]));if(window.updateMap)window.updateMap(filtered);
}
...
$('clear').onclick=()=>{fields.forEach(x=>$(x).value='Todos');$('q').value='';drill=null;page=1;render()};
