...
function renderMap(rows){
 ...
 const labels=MAP_DATA.features.map(f=>{const q=project([Number(f.properties.longitude),Number(f.properties.latitude)]);return'<text class="stateLabel" x="'+q[0]+'" y="'+q[1]+'">'+f.properties['postal-code']+'</text>'}).join('');
